...
CLOUD_DIR="/home/star/tpm/tpm-test"  # 云端目录
CLOUD_IP="milkv"        # 云端IP或域名
...
scp $CLIENT_DIR/nonce.bin $CLOUD_IP:$CLOUD_DIR/
...
if [ "$QUOTED_PCR" != "$GOLDEN_PCR" ]; then
  ...
  scp $CLIENT_DIR/auth_failed.log $CLOUD_IP:$CLOUD_DIR/
  exit 1
...
# 发送凭证到云端
scp $CLIENT_DIR/aescred.out $CLOUD_IP:$CLOUD_DIR/
